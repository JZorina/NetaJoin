...
class Staff{
	...
	function AddStaff($data)
	{
		...
		$cityid = $data->cityid;
		$email = $data->email;
		$superstaffid = $data->superstaffid;
		$type = $data->type;
		...
		if($AppUser->getUserIdByEmail($email)!=null)
		{
			return (object)array("error"=>"email already exists");
		}
		if($AppUser->GetUserIdByTz($tznumber))
		{
		...
		$result = $db->smartQuery(array(
			'sql' => "INSERT INTO `staff` (staffid,`firstname`,`firstnameinarabic`, `lastname`,`lastnameinarabic`,`tznumber`,`phone`,`adress`,`birthday`, `genderid`, `religionid`, `email`, `superstaffid`, `type`,`registerdate`,`image`,`status`,`cityid`) VALUES (:staffid,:firstname,:firstnameinarabic, :lastname,:lastnameinarabic, :tznumber, :phone,:adress,:birthday, :genderid, :religionid, :email, :superstaffid, :type, :registerdate, :image, :status, :cityid);",
				'par' => array('firstname' => $firstname,'staffid' => $staffid,'firstnameinarabic' => $firstnameinarabic,'lastname' => $lastname,'lastnameinarabic' => $lastnameinarabic,'tznumber' => $tznumber,'phone' => $phone,'adress'=>$adress,'birthday'=>$birthday, 'genderid' => $genderid, 'religionid' => $religionid, 'email' => $email, 'superstaffid' => $superstaffid, 'type' => $type, 'registerdate' => $registerdate, 'image' => $image, 'status' => $status, 'cityid' => $cityid),
				'ret' => 'result'
			));
	...
	{
		global $db;
		$id = $reportSubject->staffreportsubjectid;
		...
		if($id!='')
		{
			...
			$result = $db->smartQuery(array(
				...
				SET
					...
					projectid=:projectid
				WHERE
					staffreportsubjectid=:staffreportsubjectid
					AND staffid = :staffid",
				'par' => array( 'staffid' => $staffid, 'status' => $status, 'clientcodeid' => $clid, 'subjectreportid' => $subid, 'projectid' => $proid, 'staffreportsubjectid' => $id),
				'ret' => 'result'
			));
		}else
		{
	...
	function UpdateStaff($data)
	{
		...
		$useridByTz = $AppUser->GetUserIdByTz($tznumber);
		if($useridByTz&&$useridByTz!=$staffid)
		{
			return (object)array("error"=>"tz exist in the system");
		}
		$useridByEmail = $AppUser->getUserIdByEmail($email);
		...
		$result = $db->smartQuery(array(
			'sql' => "UPDATE `staff` SET status=:status, cityid=:cityid, firstname=:firstname, firstnameinarabic=:firstnameinarabic, lastname=:lastname, lastnameinarabic=:lastnameinarabic, tznumber=:tznumber, phone=:phone, adress=:adress, birthday=:birthday, genderid=:genderid, religionid=:religionid, email=:email, superstaffid=:superstaffid, type=:type, registerdate=:registerdate, image=:image, cityid=:cityid  WHERE staffid=:staffid;",
			'par' => array('firstname' => $firstname,'cityid' => $cityid,'status' => $status,'firstnameinarabic' => $firstnameinarabic,'lastname' => $lastname,'lastnameinarabic' => $lastnameinarabic,'tznumber' => $tznumber,'phone' => $phone,'adress'=>$adress,'birthday'=>$birthday, 'genderid' => $genderid, 'religionid' => $religionid, 'email' => $email, 'superstaffid' => $superstaffid, 'type' => $type, 'registerdate' => $registerdate, 'image' => $image, 'staffid' => $staffid, 'cityid' => $cityid),
			'ret' => 'result'
		));
	...
	function GetStaffProfileById($id)
	{
		...
		$staff = $db->smartQuery(array(
				'sql' => "
				SELECT s.*, manager.staffid AS superstaffid, CONCAT(manager.firstname, ' ', manager.lastname) AS superstaffname , sc.certificateid AS certificateid, sl.languageid, sp.professionid,
				srs.staffreportsubjectid, srs.clientcodeid, srs.subjectreportid, srs.projectid, srs.status AS reportSubjectStatus
				FROM staff AS s
				...
				LEFT JOIN staff AS manager ON manager.staffid = s.superstaffid
				WHERE
					s.staffid=:staffid",
				'par' => array('staffid'=>$id),
				'ret' => 'all'
		...
		array(
			...
			array('nestBy'=>'staffreportsubjectid', 'nestIn'=>'reportSubjects', 'fieldsToNest'=>array('staffreportsubjectid', 'clientcodeid', 'subjectreportid', 'projectid', 'reportSubjectStatus'))
		));
		if(isset($staff[0]))
		{
	...
	function GetCoursesTaughtByStaff($staffid)
	{
		global $db;
	...
	function GetSubStaffByStaffId($staffid)
	{
		global $db;
	 ...
	 * }
	 */
	function SearchStaff($search, $sorting, $desc, $userstatus, $page)
	{
		global $db;
		$ITEMS_PER_PAGE=15;
		$sortByField='staffid';
	...
	/**
		 ...
		 * }
		 */
	function SearchStaffToAssign($search, $sorting, $desc, $userstatus, $page, $excludeids=null)
	{
		global $db;
		$ITEMS_PER_PAGE=15;
		$sortByField='staffid';
	...
	function UpdateStaffProfilePic ($staffid, $image)
	{
		global $db;
		$result=$db->smartQuery(array(
	...
	function batchUploadStaff($newStaff)
	{
		...
		foreach($newStaff as $newStaffMember)
		{
			...
			$user = $AppUser->GetUserId($email,$password);
			if(isset($user))
			{
	...
	function GetActions($staffid=null)
	{
		global $db;
		global $myid;
