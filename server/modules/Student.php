...
class Student{
	...
	function UpdateStudent($data, $updatePassword)
	{
		...
		$userByEmail = $AppUser->getUserIdByEmail($email);
		if($userByEmail!=$studentid)
		{
			return (object)array("error"=>"email already exists");
		}
		$userByTz = $AppUser->GetUserIdByTz($tznumber);
	...
	function UpdateStudentGender ($studentid, $genderid)
	{
		global $db;
		$result=$db->smartQuery(array(
	...
	function UpdateStudentReligion ($studentid, $religionid)
	{
		global $db;
		$result=$db->smartQuery(array(
