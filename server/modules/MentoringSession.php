	...
	function addAutoReport($courseid, $scheduleddate)
	{
		...
		if(isset($Actions) && count($Actions)>0)
		{
			foreach($Actions as $Action)
			{
				...
				if($subject['subject']=='הנחייה אישית/קבוצתית חניכים')
				{
	...
	function AssignStudents($mentoringsessionid,$students)
	{
		...
		foreach($students as $student)
		{
			$result = $result && $db->smartQuery(array(
				'sql' => "INSERT INTO `mentoringsession_student` (`studentid`,`mentoringsessionid`) VALUES ( :studentid, :mentoringsessionid);",
				'par' => array( 'studentid' => $student, 'mentoringsessionid' => $mentoringsessionid),
				'ret' => 'result'
			));
		}
		return $result;
